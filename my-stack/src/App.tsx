...
const App = () => {
  ...
  const handleCostChange = (id: string, cost: number) => {
    setColumns((prev) => ({
      ...prev,
      paid: prev.paid.map((tool) => (tool.id === id ? { ...tool, cost } : tool)),
    }));
  };
  ...
  const handleImport = (data: Record<ColumnType, Tool[]>) => {
    const normalize = (list: Tool[], column: ColumnType) =>
      (Array.isArray(list) ? list : []).map((tool) => ({
        ...tool,
    ...
    setColumns({
      free: normalize(data.free, "free"),
      paid: normalize(data.paid, "paid"),
    });
  };
    ...
    <div className="min-h-screen bg-bg px-6 py-10 text-white">
      <div className="mx-auto flex w-full max-w-[1200px] flex-col gap-8">
        ...
        >
          ...
          <DragOverlay>
            {activeTool ? (
              <div className="drag-overlay flex items-center gap-3 rounded-lg border border-border bg-panel px-3 py-3 text-sm text-white card-shadow">
                <img
